...
<style>
  ...
  <div class="container w-50 border mt-5">
    <!-- add category -->
        <div class="title mb-4">Edit Category</div>
        <form id="edit_categories" method="POST">
            @method('PUT')
            @csrf
            <div class="row">
                <div class=" col-6 mb-3">
                    <label for="category_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="category_name" name="category_name" value="{{isset($category)? $category->category_name : ''}}">
                    <span class="text-danger category_name"></span>
                </div>
                <div class="col-6 mb-3">
                    <label for="category_slug" class="form-label">Slug</label>
                    <input type="text" class="form-control" name="category_slug" id="category_slug" value="{{isset($category)? $category->category_slug : ''}}">
                    <span class="text-danger category_slug"></span>
                </div>
            ...
            <div class="col-12 mb-3">
                ...
                <textarea class="form-control" id="category_description" name="category_description" rows="3">{{isset($category)? $category->category_description : ''}}</textarea>
                <input type="hidden" id="category_id" name="category_id" data-category_id="{{$category->id}}" value="{{$category->id}}">
                <span class="text-danger category_description"></span>
            </div>
            <div class="button">
                <input type="submit" value="Category Save">
            </div>
        </form>
